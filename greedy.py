...
def solve_greedy(data, alpha, beta):
    n_mut = data.shape[0]
    ...
    start_llh = round(tree.conditional_llh(data, alpha, beta), 4)
    ...
    while(True):
        ...
        for j in range(len(all_spr)):
            ...
            next_llhs_spr[j] = new_tree.conditional_llh(data, alpha, beta)
        ...
        for k in range(len(all_swaps)):
            ...
            next_llhs_swap[k] = new_tree.conditional_llh(data, alpha, beta)
        ...
        current_llh = round(tree.conditional_llh(data, alpha, beta), 4)
        
    return start_llh, current_llh
