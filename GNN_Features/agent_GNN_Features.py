...
Transition = namedtuple('Transition', ('state_action', 'next_state_actions', 'reward', 'done', 'num_actions'))
...
class Agent_GNN_Features():
    ...
    def predict_step_soft(self, data_batch, temperature):
    ...
    def predict_step_epsilon(self, data_batch, eps_threshold):
        ...
        else: return torch.tensor([[random.randint(0, data_batch.num_graphs-1)]], device=self.device, dtype=torch.long)

    def predict_step_epsilon_soft(self, data_batch, temperature, eps_threshold):
        sample = random.random()
        if sample > eps_threshold:
    ...
    def get_max_next_state_action_values(self, next_state_action_batch, num_actions_per_item):
        data_batch = Batch.from_data_list(next_state_action_batch)
        with torch.no_grad():
            q_vals = self.policy_net(data_batch.x, data_batch.edge_index, data_batch.batch)
    ...
    def __optimize_model(self, memory, optimizer):
        ...
        state_action_batch = Batch.from_data_list(batch.state_action)
        reward_batch = torch.cat(batch.reward)
        next_state_action_batch = [data for data_list in batch.next_state_actions for data in data_list]
        num_actions_batch = batch.num_actions
        done_batch = torch.cat(batch.done)
        not_done_mask = ~done_batch
        #state_action_values = self.get_state_action_values(state_action_batch)
        state_action_values = self.policy_net(state_action_batch.x, state_action_batch.edge_index, state_action_batch.batch).squeeze()
        max_next_state_action_values = self.get_max_next_state_action_values(next_state_action_batch, num_actions_batch)
        ...
        for e in range(P.EPISODES):
            for i in range(len(data_train)):
                ...
                for _ in range(P.HORIZON):                
                    ...
                    next_graph_data = self.get_graph_data(next_state, next_actions)
                    memory.push(graph_data[action], next_graph_data, reward, done, num_actions)
