...
class Agent():
    ...
    def get_state_actions(self, state, actions):
        ...
        state_exp = state.expand(actions.shape[0], -1)
        return torch.cat((state_exp, actions), dim=1)
    ...
    def predict_step_epsilon(self, state_actions, matrix, eps_threshold):
        ...
        if sample > eps_threshold: return self.predict_step(state_actions, matrix)
    ...
    def train_net(self, data_path, batch_size, episodes, lr = 1e-4, gamma = 0.99, tau = 0.005):
        ...
        for e in range(episodes): 
            for i in range(len(data_train)):
                ...
                state_actions = self.get_state_actions(state, actions)
                mse = 0
                for t in count():
                    ...
                    next_state_actions = self.get_state_actions(next_state, next_actions)
