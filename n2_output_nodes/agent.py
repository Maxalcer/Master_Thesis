...
class Agent():
    def __init__(self, n_mut, n_cells, alpha, beta, device = "cuda"):
        ...
        dim = n_mut*(n_mut+1)
        self.policy_net = DQN(dim, dim).to(self.device)
        self.target_net = DQN(dim, dim).to(self.device)
    ...
    def __optimize_model(self, memory, batch_size, gamma, optimizer):
        ...
        with torch.no_grad():
            ...
            max_next_state_action_values = self.target_net(next_state_batch).gather(1, next_actions).squeeze()
