...
class MutationTree(PruneTree):
    ...
    def cell_attatchment(self, data, alpha, beta):
        p= [1-alpha, beta, alpha, 1-beta]
        A_T = self.ancestor_matrix()

        sig = np.zeros(self.n_cells, dtype=int)

        for j in range(self.n_cells):
            lh = np.ones(self.n_mut+1)
            for k in range(self.n_mut+1):
                for i in range(self.n_mut):
                    lh[k] *= p[int((data[i,j] * 2) + (A_T[i, k]))]            
            sig[j] = np.argmax(lh)

        return sig

    def conditional_lh(self, data, alpha, beta):
        p= [1-alpha, beta, alpha, 1-beta]
        A_T = self.ancestor_matrix()
        ...
        for i in range(self.n_mut):
            for j in range(self.n_cells):
                lh *= p[int((data[i,j] * 2) + (A_T[i, sig[j]]))]
        return lh
