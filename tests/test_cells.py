...
all_data = read_data("/home/mi/maxa55/Master_Thesis/Data/test/n_cells", noisy=True, validation= True)
all_newick = read_newick("/home/mi/maxa55/Master_Thesis/Data/test/n_cells", validation= True)

matrix_paths = glob.glob("/home/mi/maxa55/Master_Thesis/Data/test/n_cells/valid*.noisy")
...
for i in range(len(all_data)):
    ...
    results.append({'Method': "FCNN-3", 'Num_Cells': n_cells, 'Perf': perf_feat})
    results.append({'Method': "GNN-4", 'Num_Cells': n_cells, 'Perf': perf_gnn})
    results.append({'Method': "Greedy", 'Num_Cells': n_cells, 'Perf': perf_greedy})
    results.append({'Method': "SCITE", 'Num_Cells': n_cells, 'Perf': perf_scite})

df = pd.DataFrame(results)
df.to_csv("/home/mi/maxa55/Master_Thesis/Results/Plots/Num_Cells.csv", index=False)


plt.figure(figsize=(10, 6))
sns.boxplot(x='Num_Cells', y='Perf', hue='Method', data=df)
plt.title('Performance Comparison by Number of Cells')
plt.ylabel('Performance')
plt.xlabel('Number of Cells')
plt.legend(title='Method')
plt.tight_layout()
plt.savefig("/home/mi/maxa55/Master_Thesis/Results/Plots/Num_Cells.png")
plt.show()